  ...
  return (
    ...
    >
      ...
      <div className="relative p-4 md:p-5 lg:p-6">
        <div className="flex flex-col gap-4 lg:grid lg:grid-cols-[minmax(320px,400px)_minmax(0,1fr)_250px] lg:gap-5">
          <div
          ...
          >
            <div className="absolute inset-x-0 top-0 z-10 bg-gradient-to-b from-[#8f793d]/18 via-[#8f793d]/6 to-transparent p-3 sm:p-4">
              <div className="rounded-[22px] border border-[rgba(201,161,59,0.26)] bg-[linear-gradient(180deg,rgba(255,252,246,0.94),rgba(244,234,214,0.9))] p-3 text-text-primary shadow-[0_18px_44px_rgba(88,66,18,0.16)] backdrop-blur-md sm:p-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <span className="rounded-full border border-[rgba(201,161,59,0.26)] bg-[rgba(201,161,59,0.12)] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-brand-goldDark">
                      {group.powertrain === 'BEV' ? 'Electric Drive' : group.powertrain === 'PHEV' ? 'Plug-in Hybrid' : 'Nowy model'}
                    </span>
                    {yearLabel ? (
                      <span className="rounded-full border border-stroke/80 bg-white/76 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-text-secondary shadow-[0_8px_18px_rgba(19,26,43,0.05)]">
                        Rocznik {yearLabel}
                      </span>
                    ) : null}
                  </div>

                  <span className="rounded-full border border-[#c9a13b]/45 bg-[linear-gradient(135deg,rgba(235,201,113,0.92),rgba(182,132,28,0.88))] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-white shadow-[0_10px_24px_rgba(165,125,31,0.2)]">
                    Dostępny od ręki
                  </span>
                </div>
                ...
                <div className="mt-3 rounded-2xl border border-stroke/75 bg-white/72 px-3 py-2.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.55),0_8px_18px_rgba(88,66,18,0.05)]">
                  ...
                  <div className="mt-2 flex flex-wrap gap-1.5">
                    {versionNames.map((trim) => (
                      <span key={trim} className="rounded-full border border-brand-gold/20 bg-white/90 px-2.5 py-1 text-[10px] font-semibold text-text-primary">
                        {trim}
                      </span>
                    ))}
                    {extraVersionNames > 0 ? (
                      <span className="rounded-full border border-brand-gold/20 bg-[#fbf4e6] px-2.5 py-1 text-[10px] font-semibold text-[#7b5a15]">
                        +{extraVersionNames}
                      </span>
                    ) : null}
                  </div>
            ...
            <div className="relative aspect-[16/10] min-h-[220px] sm:min-h-[260px] lg:h-full lg:min-h-[280px] lg:aspect-auto">
              <Image
                src={imgSrc}
                alt={`${group.brand} ${group.model}`}
                fill
                sizes="(max-width: 768px) 100vw, 420px"
                className="object-contain px-5 pb-4 pt-12 transition-transform duration-500 group-hover:scale-[1.035]"
                style={{ objectPosition: 'center 75%' }}
                priority={false}
              />
            </div>

            <div className="absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-[#f6ecda]/95 via-[#f8f1e4]/78 to-transparent p-3 sm:p-4">
          ...
          <div className="flex min-w-0 flex-1 flex-col justify-between rounded-[24px] border border-white/60 bg-white/72 p-5 backdrop-blur-sm lg:p-6">
            <div>
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[#8b6a21]">Najważniejsze</p>
                  <h3 className="mt-2 text-[24px] font-semibold leading-tight tracking-tight text-text-primary sm:text-[28px]">
              ...
              <div className="mt-6 rounded-[20px] border border-stroke/80 bg-[#fffdfa] p-4 shadow-[0_10px_24px_rgba(88,66,18,0.04)]">
                ...
                <div className="mt-3 grid gap-2">
                  {strengthsPreview.length ? (
                    strengthsPreview.map((item) => (
                      <div key={item} className="rounded-2xl border border-stroke/75 bg-white px-3 py-3 text-sm font-medium text-text-primary shadow-[0_8px_18px_rgba(19,26,43,0.04)]">
                        {item}
                      </div>
                    ))
                  ) : (
          ...
          <div className="flex flex-col justify-between rounded-[24px] border border-[#ead9b4] bg-[linear-gradient(180deg,#fffdf8_0%,#f4ead7_100%)] p-5 text-text-primary shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] lg:min-h-full">
            <div>
            ...
            <span className="mt-5 inline-flex h-11 items-center justify-center rounded-md border border-[#b6841c] bg-[linear-gradient(135deg,#ebc971,#b6841c)] px-4 text-sm font-semibold text-white shadow-[0_14px_28px_rgba(182,132,28,0.24)] transition group-hover:shadow-[0_18px_34px_rgba(182,132,28,0.32)]">
              Sprawdź szczegóły
            </span>
          </div>
        </div>
