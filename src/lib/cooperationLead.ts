...
export type CooperationLeadInput = {
  name: string
  email: string
  phone: string
  company: string
...
export type CooperationLeadRecord = CooperationLeadInput & {
  leadId: string
  createdAt: string
}
...
const COOPERATION_COLUMNS_RANGE = 'A:L'
...
async function withSheetsRetry<T>(operation: string, task: () => Promise<T>, attempts = 3): Promise<T> {
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      ...
      console.warn(`[cooperation:sheets-retry] ${operation} attempt ${attempt} failed, retrying in ${delayMs}ms`, error)
      await sleep(delayMs)
    }
  }
...
async function createSheetClient(): Promise<SheetClient> {
  const spreadsheetId = resolveSpreadsheetId(
    env('GOOGLE_COOPERATION_SHEET_ID') || env('GOOGLE_WEBINAR_SHEET_ID') || env('GOOGLE_PARTNER_SIGNUPS_SHEET_ID'),
  )
  if (!spreadsheetId) {
    throw new Error('Brak GOOGLE_COOPERATION_SHEET_ID, GOOGLE_WEBINAR_SHEET_ID lub GOOGLE_PARTNER_SIGNUPS_SHEET_ID.')
  }
...
function getCooperationSheetTab() {
  return env('GOOGLE_COOPERATION_SHEET_TAB') || 'Wspolpraca'
}
          ...
          },
          {
            repeatCell: {
              range: {
                ...
                endRowIndex: 1,
                startColumnIndex: 0,
                endColumnIndex: COOPERATION_HEADERS.length,
              },
              cell: {
                userEnteredFormat: {
          ...
          },
          {
            autoResizeDimensions: {
              dimensions: {
                ...
                dimension: 'COLUMNS',
                startIndex: 0,
                endIndex: COOPERATION_HEADERS.length,
              },
            },
          },
        ],
...
async function ensureSheetTabExists(client: SheetClient, tabName: string) {
  ...
  await withSheetsRetry(`ensureSheetTabExists:header:${tabName}`, () =>
    client.sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [COOPERATION_HEADERS] },
    }),
  )
...
async function ensureSheetTabStructure(client: SheetClient, tabName: string) {
  ...
  const normalizedExpectedHeader = COOPERATION_HEADERS.map(normalizeHeader)
  ...
  const preservedRows = rows.slice(1).map((row) => {
    const normalizedRow = row.slice(0, COOPERATION_HEADERS.length).map((value) => String(value ?? ''))
    while (normalizedRow.length < COOPERATION_HEADERS.length) {
      normalizedRow.push('')
    }
  ...
  await withSheetsRetry(`ensureSheetTabStructure:update:${tabName}`, () =>
    client.sheets.spreadsheets.values.update({
      ...
      valueInputOption: 'RAW',
      requestBody: { values: [COOPERATION_HEADERS, ...preservedRows] },
    }),
  )
...
async function ensureRequiredSheetTab(client: SheetClient) {
  ...
  ensuredSheetPromise = (async () => {
    await ensureSheetTabExists(client, getCooperationSheetTab())
    await ensureSheetTabStructure(client, getCooperationSheetTab())
...
function toSheetRow(record: CooperationLeadRecord): Array<string | number> {
  return [
    record.createdAt,
    record.name,
    record.email,
...
async function appendRowToTab(client: SheetClient, tabName: string, row: Array<string | number>) {
  await withSheetsRetry(`appendRowToTab:${tabName}`, () =>
    client.sheets.spreadsheets.values.append({
      spreadsheetId: client.spreadsheetId,
      range: `${tabName}!${COOPERATION_COLUMNS_RANGE}`,
...
async function sendDiscordNotification(record: CooperationLeadRecord) {
  const webhookUrl = env('DISCORD_COOPERATION_WEBHOOK_URL') || env('DISCORD_PARTNER_SIGNUPS_WEBHOOK_URL')
  if (!webhookUrl) {
    return
  }

  const message = [
    'Nowe zgłoszenie współpracy Velo Prime',
    `Imię i nazwisko: ${record.name}`,
    `Telefon: ${record.phone}`,
    `E-mail: ${record.email}`,
    `Firma: ${record.company || '—'}`,
    `Miasto / region: ${record.city || '—'}`,
...
export async function registerCooperationLead(input: CooperationLeadInput) {
  const record: CooperationLeadRecord = {
    ...input,
    leadId: randomUUID(),
  ...
  await appendRowToTab(client, getCooperationSheetTab(), toSheetRow(record))

  const notifications = await Promise.allSettled([sendDiscordNotification(record)])
  notifications.forEach((result) => {
    if (result.status === 'rejected') {
      console.error('[cooperation:discord]', result.reason)
    }
  })
